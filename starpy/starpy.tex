\chapter{STARPY: Bayesian inference of a galaxy's star formation history}\label{starpy}
...
Frequentist (or traditional) statistics allows you to test whether an event (i.e. a hypothesis) occurs or not and calculate the probability of it occurring over many trials of an experiment. The probability derived is also biased by the number of experiment trials conducted. Conversely, a Bayesian approach allows you to directly test whether a hypothesis, $\theta$, is true, given the data you already have, $d$, by relating this to something you can easily calculate: the probability that you would observe the data if the hypothesis was true. It does so by cleverly employing the rules of conditional probability into what's known as Bayes' theorem:
\begin{equation}\label{bayes}
P(\theta|d) \propto P(d | \theta)P(\theta),
\end{equation}
which is made up of three separate terms: 
\begin{enumerate}[(i)]
\item $P(\theta)$, known as the \emph{prior} probability and represents your knowledge (or ignorance) about the hypothesis before you have analysed any data. 
...
\end{enumerate}
The missing factor in Equation~\ref{bayes} is just a normalisation constant and doesn't explicity depend on your hypothesis and so doesn't have to be calculated for model parameter estimation problems (i.e. your hypothesis, $\theta$, is a model described by some number of parameters with the goal of emulating your data). Deciding on a prior and likelihood functions is very much dependent on the problem you are trying to solve and are often informed by your knowledge of the problem. 

This difference between frequentist and Bayesian statistics is often highlighted by the age old statistical example of a coin being tossed; if I toss a coin 10 times and get 3 heads ($H=3$) and 7 tails ($T=7$) what is the probability that the coin is weighted? With frequentist statistics, you are limited heavily by the fact that I only tossed the coin 10 times; I haven't repeated the experiment enough for you to be sure of your response that the probability of tossing a tails,  $P(T)=0.7$. Perhaps if I'd tossed the coin a $1000$ times and recorded $720$ tails then you could be more certain that the coin was weighted. Herein lies the problem of frequentist statistics; not only is your answer dependent on the number of experiment trails but you also can't quantify how likely your answer is of being correct. 
...
As the denominator of Equation~\ref{big} is a normalisation factor, comparison between likelihoods for two different SFH models (i.e., two different combinations of $\theta_k = [t_q, \tau]$) is equivalent to a comparison of the numerators. Markov Chain Monte Carlo (MCMC; \citealt{mackay03, emcee13, GW10}) provides a robust comparison of the likelihoods between $\theta$ values.
